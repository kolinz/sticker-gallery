...
```bash
# 1. Node.js / npm のパスを確認する（nvm 環境ではパスが /usr/bin/ とは異なる）
which node
...
# 2. sticker-gallery.service の ExecStart を実際の npm パスに書き換える
sed -i "s|ExecStart=/usr/bin/npm start|ExecStart=$(which npm) start|" sticker-gallery.service
