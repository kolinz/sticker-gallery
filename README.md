...
# 3. Node.js / npm のパスを確認する（nvm 環境ではパスが /usr/bin/ とは異なる）
which node
# 例（nvm）: /home/ubuntu/.nvm/versions/node/v24.14.1/bin/node
# 例（apt）: /usr/bin/node

which npm
# 例（nvm）: /home/ubuntu/.nvm/versions/node/v24.14.1/bin/npm
# 例（apt）: /usr/bin/npm

# 4. アプリを本番パスに配置する（開発環境のパスとは別に本番用パスを用意する場合）
#    例: /opt/sticker-gallery に配置する場合
sudo cp -r . /opt/sticker-gallery
sudo chown -R ubuntu:ubuntu /opt/sticker-gallery   # ← whoami の結果のユーザー名に変更
#    ※ 既に本番パスで作業している場合はこの手順は不要

# 5. sticker-gallery.service の ★ 箇所を実際の値に書き換える
#    （テキストエディタで開いて User= / WorkingDirectory= / ExecStart= / EnvironmentFile= を編集）
vi sticker-gallery.service
